...
class MyDataset(Dataset):
    def __init__(self, train=True, transform=None, noise_transform=None, add_noise=True):
    ...
    def __getitem__(self, index):
        ...
        if self.add_noise and self.noise_transform:
...
def get_cifar10_dataloaders(batch_size, noise_type='gaussian', noise_params={}, add_noise=True):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize([0.4914, 0.4822, 0.4465], [0.247, 0.243, 0.261])
    ])
    noise_transform = AddNoise(noise_type, noise_params) if add_noise else None
    
    train_dataset = MyDataset(train=True, transform=transform, noise_transform=noise_transform, add_noise=add_noise)
    test_dataset = MyDataset(train=False, transform=transform, noise_transform=noise_transform, add_noise=add_noise)
