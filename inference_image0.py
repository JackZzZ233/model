...
def get_image0(args):
    ...
    if args.input_selection == "use_stl10_image0":
        stl10_np = np.load("C:/Users/Administrator/Downloads/train.npz")['x']
    ...
    if args.input_selection == "use_cifar10_image0":
        cifar10_np = np.load("C:/Users/Administrator/Downloads/train.npz")['x']
        if args.model_type == "styleganv2ada_cifar10":
            cifar10_np_y = np.load("C:/Users/Administrator/Downloads/train.npz")['y']
    ...
    if args.input_selection == "use_imagenet_image0":
        imagenet_dir = "C:/Users/Administrator/Downloads/train.npz"
    ...
    if args.filter_type!=None:
        image0 = apply_filter_to_image(image0, filter_type=args.filter_type, kernel_size=args.kernel_size)
        save_img_tensor(image0, "image0_with_filter.png")
...
#add_noise to image
def add_noise(image,std,mean,noise_type,salt_prob):
...
#add_filter to image
def apply_filter_to_image(image, filter_type, kernel_size=5):

    image_np = image.squeeze(0).cpu().numpy().transpose(1, 2, 0)  

    if filter_type == 'blur':
        filtered_image = pilgram.aden(image_np)
    elif filter_type == 'sharpen':
        kernel = np.array([[0, -1, 0], [-1, 5, -1], [0, -1, 0]])
        filtered_image = cv2.filter2D(image_np, -1, kernel)
    elif filter_type == 'edge':
        gray_image = cv2.cvtColor(image_np, cv2.COLOR_RGB2GRAY)
        edges = cv2.Canny((gray_image * 255).astype(np.uint8), 100, 200) / 255.0
        filtered_image = np.stack([edges, edges, edges], axis=-1)  
    elif filter_type == 'emboss':
        kernel = np.array([[-2, -1, 0], [-1, 1, 1], [0, 1, 2]])
        filtered_image = cv2.filter2D(image_np, -1, kernel)

    filtered_image = torch.tensor(filtered_image.transpose(2, 0, 1)).unsqueeze(0).cuda().float() 
